interface Document {
    getElementById(elementId: string): HTMLInputElement;
}

interface Window {
    diceAudioContext?: AudioContext;
    diceTimeout?: ReturnType<typeof setTimeout>;
...
// Nomes das perícias sem '+' e '*'
    ...
    function applyClassBenefits() {
        ...
        if (allChoicesSelected) closeClassMenu();
        calculateSheet(true);
    }
    ...
    function updateClassSelection() {
        ...
        if (!primarySelect || !evolutionSelect || !classInput) return;
        ...
        if (trigger) trigger.innerText = selectedClass || 'Selecionar';
    }
    ...
    function renderClassMenu(showEvolutions = false) {
        ...
        const primarySelect = document.getElementById('char-class-primary');
        if (!grid || !title || !primarySelect) return;
    ...
    function closeClassMenu(event?: Event) {
    ...
    function updateClassEvolutionOptions() {
        ...
        if (!primarySelect || !evolutionSelect || !evolutionButton) return;

        const selectedClass = primarySelect.value;
        const primary = Object.keys(classEvolutions).includes(selectedClass) ? selectedClass : primarySelect.dataset.primary || Object.keys(classEvolutions).find(className =>
            className === selectedClass || classEvolutions[className].includes(selectedClass)
        ) || selectedClass;
    ...
    function toggleClassEvolution(event) {
        event.stopPropagation();
        if (evolutionButtonIsVisible()) toggleClassMenu(event, true);
    }
    ...
    function toggleAttackSelector(event, id) {
        ...
        document.querySelectorAll('.attack-selector-container').forEach(container => {
            if (container.id !== `${id}-container`) container.classList.remove('active');
        });
    ...
    function showDiceResult(title, details, total, soundType = '') {
        playDiceSound();
        if (soundType) playAttackSound(soundType);
    ...
    function closeAccountMenu() { document.getElementById('account-modal').style.display = 'none'; }
    ...
    async function loadAccountSheets() {
        ...
        try {
            ...
            const result = await response.json();
            if (!response.ok) throw new Error(result.error || 'Não foi possível carregar as fichas.');
            list.innerHTML = result.sheets.map(sheet => `<div class="saved-sheet-row"><div><strong>${escapeAdminText(sheet.name)}</strong><small>Atualizada em ${escapeAdminText(new Date(sheet.updatedAt).toLocaleString('pt-BR'))}</small></div><button type="button" onclick="loadSavedAccountSheet('${escapeAdminText(sheet.id)}')">Carregar</button></div>`).join('') || '<p class="history-empty">Nenhuma ficha salva nesta conta.</p>';
        } catch (error) {
    ...
    async function loadSavedAccountSheet(sheetId: string) {
        const feedback = document.getElementById('account-feedback');
        try {
    ...
    function rollAttack(button) {
        ...
        showDiceResult(
            title,
            `Acerto: d20 [${d20}] + atributo (${attributeValue * 2}) + bônus (${attackBonus}) = ${attackTotal} | Dano: ${damageRolls} ${damage.bonus >= 0 ? '+' : ''}${damage.bonus}${criticalText}`,
            damage.total,
            attack.dataset.type
        );
    }
    ...
    function saveAttack() {
        ...
        `;
        if (!editingAttack) container.appendChild(item);
        editingAttack = null;
        document.getElementById('attack-modal-title').innerText = 'Novo Ataque';
        document.getElementById('attack-save-btn').innerText = 'Adicionar';
    ...
    function saveAbility() {
        ...
        `;
        if (!editingAbility) container.appendChild(item);
        editingAbility = null;
        document.getElementById('ability-modal-title').innerText = 'Nova Habilidade';
        document.getElementById('ability-save-btn').innerText = 'Adicionar';
    ...
    function collectSheetData() {
        ...
        document.querySelectorAll('input[id], textarea[id], select[id]').forEach(field => {
            if (field.type !== 'file') fields[field.id] = field.value;
        });
    ...
    function restoreSheetData(sheetData) {
        ...
        if (primarySelect && !primarySelect.value && savedClass) {
            const savedPrimary = Object.keys(classEvolutions).find(primary =>
                primary === savedClass || classEvolutions[primary].includes(savedClass)
            );
            if (savedPrimary) {
